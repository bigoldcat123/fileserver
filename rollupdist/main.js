#!/usr/bin/env node
import require$$1$1, { statSync, readdirSync } from 'fs';
...
var send$1 = /*@__PURE__*/getDefaultExportFromCjs(sendExports);
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        console.log('watching at : ' + path$1.resolve(_path));
        createServer((req, res) => {
            try {
                const decodeUrl = decodeURIComponent(req.url);
                if (decodeUrl.includes('/favicon.ico')) {
                    send$1(req, path$1.join(import.meta.dirname, 'favicon.ico')).pipe(res);
                    return;
                }
                const dir = path$1.join(_path, decodeUrl);
                const files = [];
                if (!statSync(dir).isDirectory()) {
                    send$1(req, dir).pipe(res);
                    return;
                }
                const navigations = [];
                const paths = path$1.join(decodeUrl).split(path$1.sep);
                let url = '';
                paths.forEach(x => {
                    if (!x)
                        return;
                ...
                files.sort((a, b) => {
                    if (statSync(path$1.join(dir, a)).isDirectory() && !statSync(path$1.join(dir, b)).isDirectory()) {
                        return -1;
                    }
                    if (statSync(path$1.join(dir, b)).isDirectory() && !statSync(path$1.join(dir, a)).isDirectory()) {
                        return 1;
                    }
                    return 0;
                });
                ...
                <html>
                    ...
                    <span><a href='/'>root/</a></span>${navigations.map(x => `<span><a href="${x.url}">${x.name}</a> </span>`).join('/')}</div>
                        <ul>
                            ${files.map(x => `<li><a ${statSync(path$1.join(dir, x)).isDirectory() ? '' : `download="${x}"`}  href="${path$1.join(decodeUrl, x).replace(/\\/g, '/')}">${x}</a></li>`).join('')}
                        </ul>
                    </body>
                </html>
                `);
            ...
            console.log('listen' + 'http://' + locatIp$1 + ':' + port);
        });
    });
}
main(); //---
